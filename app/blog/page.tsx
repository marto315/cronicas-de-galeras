      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-8">
        <div>
          ...
          <div className="mb-8 flex flex-wrap gap-2">
            <Link
              href="/blog"
              className="rounded-full bg-clay px-4 py-1.5 text-sm font-medium text-cream"
            >
              Todas
            </Link>
            {CATEGORIES.map((cat) => (
              <Link
                ...
                className="rounded-full border border-sand bg-cream px-4 py-1.5 text-sm font-medium text-ink/80 transition hover:border-clay hover:text-clay"
              >
                {cat.label}
              </Link>
            ))}
